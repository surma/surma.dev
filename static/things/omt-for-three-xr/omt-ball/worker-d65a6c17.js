...
define("./worker-d65a6c17.js",['./comlink-759ccaeb'], function (comlink) { 'use strict';
  ...
  class BallShooter {
    ...
    _shootBalls(delta) {
      for(const gun of this._guns) {
        ...
        for(let i = previousBallCounter; i < Math.floor(this._ballCounter); i++) {
          const ball = this._balls[i];
          vectorSet(ball.position, gun.position);
          vectorSet(ball.velocity, [random(-.1, .1), 10, random(-.1, .1)]);
          console.log("Firing ball", ball.position);
        }
      }
    }
