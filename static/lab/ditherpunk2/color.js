...
function ditherImage(image, opts) {
  return new Promise((resolve) => {
    ...
    worker = new Worker(new URL("./color-worker.js", import.meta.url), {
      type: "module",
    });
...
async function getImage() {
  switch (
    [...sources.querySelectorAll("input[type=radio]")].filter(
      (x) => x.checked
    )[0].value
  ) {
    case "custom":
      ...
      return {
        ...
        imgdata: await blobToImageData(src),
      };
      break;
    case "example":
      return {
        url: exampleimg.src,
        imgdata: await imageToImageData(exampleimg),
      };
      break;
  }
}
