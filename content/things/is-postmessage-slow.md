...
I simplified this to catch spirit of the algorithm, so it’s _technically_ incorrect. For example, `StructuredSerialize()` and `StructuredDeserialize()` are not real functions in the sense that they are not exposed via JavaScript ([yet][exposing structured clone]). What do these two functions actually do? For now, **you can think of `StructuredSerialize()` and `StructuredDeserialize()` as smarter versions of `JSON.stringify()` and `JSON.parse()`**, respectively. They are smarter in the sense that they handle cases like cyclical data structures and can also transfer built-in data types like `Map`, `Set` and `ArrayBuffer`. These smarts come at a cost, of course. Structured cloning an object is generally slower than `JSON.parse(JSON.stringify())`’ing it. Let’s put this little fun fact off to the side for now.
