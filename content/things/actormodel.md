{
  "title": "Architecture on the Web: Actor Model",
  "date": "2017-11-01",
...
The whole reason I started to think about Actors is because I started to playe around with the programming language [Pony]. Pony is a fairly new and modern language that uses the Actor Model as its core principle (together with a proper type system and capapbilty-based security). Pony is not the only language to use the Actor Model. Arguably the most popular language using the Actor Model is [Erlang], but others include [Elixir] or [Scala].

So I started thinking if and how the Actor Model would work on the web. The more I thought about it, the more it seems like a natural fit. I’m not saying that y’all need to adopt this pattern and call it the holy grail, but at the very least it’s an interesting thought experiment.

## The Actor Model

The Actor Model is a mathematical model for concurrent systems in compute science. In other words: The Actor Model gives you rules and primitives to build parallel programs. The rules are as follows:

- Everything is an actor
- An actor can have local state (“variables”)
...
- An actor can receive messages and react to them with:
  ...
  - Creating new actors
  - Sending messages to other actors asynchronously, if it has their address (“fire-and-forget”)

While each actor is run sequentially, i.e. only one message is processed by an actor at a time, two actors _can_ very well be run in parallel. 

Now that’s neither a lot of rules nor a lot of tools to have at your disposal, but it’s enough to build incredibly complex systems. For example: [Erlang], built by Joe Armstrong at Erricson, [is used today][Erlang story] for extremely scalable and fault-tolerant systems like the backbones of your LTE connection, but also in products like WhatsApp, Facebook and companies like Amazon.

The interesting thing about the Actor Model is that **it doesn’t need any locks or synchronization primitives**. No actor can modify another actors local state and each actor in itself is single-threaded so that it can’t have any race conditions. Any resource that is needed by multiple actors should get its own designated actor that manages access to this resource. All the other actors can request operations to be performed by sending a message to the managing actor.

## … on the web?

The rules outlined above fit the web perfectly. Well, at least if you squint a bit. Allow me to explain:

First of all: Let’s ignore the “Everything is an actor” bit. The main thread’s JavaScript context is the first actor, it is single-threaded, responds to messages in the form of events and has sole access to the DOM and therefore the UI. Most apps currently only use that one actor. So far, so good. What about additional actors? I think [WebWorkers][WebWorker] take the role general-purpose actors that can be created by the developer from the main actor. The new actor’s address is made available in form of a handle that they can use to send messages via `postMessage`. The worker actor can’t access any variables from the main actor and vice versa, but can listen for incoming messages and reply with messages of its own. A worker actor can also create additional actors using `new Worker()`.
